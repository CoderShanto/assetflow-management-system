...
function StatusBadge({ status }) {
  const cfg = {
    ...
    REJECTED: { bg:"rgba(239,68,68,0.1)",  border:"rgba(239,68,68,0.25)",  color:"#fca5a5" },
    PENDING:  { bg:"rgba(245,158,11,0.12)",border:"rgba(245,158,11,0.25)", color:"#fcd34d" },
...
const COLS = ["ID","Product Type","Reason","Status","Requested On"];
...
export default function MyRequestsPage() {
  ...
  const fetchMyRequests = async () => {
    try { setLoading(true); const res = await API.get("/requests/my"); setRequests(res.data); }
    catch (err) { console.error(err); alert("Failed to load your requests"); } finally { setLoading(false); }
  };
  ...
  return (
    <>
      ...
      <div style={{ display:"flex", minHeight:"100vh", background:T.bg, fontFamily:"'Sora',sans-serif" }}>
        <Sidebar active="requests" navigate={navigate} />
        <main style={{ flex:1, padding:"36px 28px", overflowY:"auto", minWidth:0 }}>
          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:28 }}>
            <div>
              <p style={{ fontSize:11, color:"#10b981", fontFamily:"'JetBrains Mono',monospace", textTransform:"uppercase", letterSpacing:"1.5px", marginBottom:6 }}>Employee · Requests</p>
              <h1 style={{ fontSize:28, fontWeight:800, color:T.text, letterSpacing:"-0.5px" }}>My Requests</h1>
              <p style={{ fontSize:13, color:T.muted, marginTop:5 }}>Track all your submitted product requests</p>
            </div>
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={fetchMyRequests} style={{ padding:"9px 16px", borderRadius:10, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"'Sora',sans-serif", background:"rgba(255,255,255,0.04)", border:`1px solid ${T.border}`, color:T.muted }} onMouseEnter={e=>e.currentTarget.style.color=T.text} onMouseLeave={e=>e.currentTarget.style.color=T.muted}>↻ Refresh</button>
              <button onClick={()=>navigate("/employee/request-product")} style={{ padding:"9px 18px", borderRadius:10, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"'Sora',sans-serif", background:"linear-gradient(135deg,#10b981,#059669)", border:"none", color:"#fff", boxShadow:"0 4px 14px rgba(16,185,129,0.3)" }}>+ New Request</button>
          ...
          <div style={{ display:"flex", gap:14, marginBottom:22, flexWrap:"wrap" }}>
            {[{label:"Total",val:counts.ALL,color:"#6ee7b7",bg:"rgba(16,185,129,0.08)"},{label:"Pending",val:counts.PENDING,color:"#fcd34d",bg:"rgba(245,158,11,0.08)"},{label:"Approved",val:counts.APPROVED,color:"#34d399",bg:"rgba(16,185,129,0.08)"},{label:"Rejected",val:counts.REJECTED,color:"#fca5a5",bg:"rgba(239,68,68,0.08)"}].map(s=>(
          ...
          <div style={{ background:T.surface, borderRadius:18, border:`1px solid ${T.border}`, overflow:"hidden" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 20px", borderBottom:`1px solid ${T.border}`, flexWrap:"wrap", gap:10 }}>
              ...
              <span style={{ fontSize:12, color:T.muted }}>{filtered.length} requests</span>
            </div>
              ...
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                ...
                <tbody>
                  {loading ? Array.from({length:4}).map((_,i)=><tr key={i}>{COLS.map(c=><td key={c} style={{ padding:"13px 16px" }}><div style={{ height:13, borderRadius:6, background:"rgba(255,255,255,0.04)", width:"80px" }} /></td>)}</tr>) :
                   filtered.length===0 ? <tr><td colSpan="5" style={{ padding:"48px 16px", textAlign:"center", color:T.muted, fontSize:14 }}>No requests found</td></tr> :
                   filtered.map((req,idx)=>(
                    <tr key={req.id} className="trow" style={{ borderBottom:`1px solid ${T.border}`, background:idx%2===0?"transparent":T.surfaceAlt+"55" }}>
                      ...
                      <td style={{ padding:"13px 16px" }}><span style={{ fontSize:11, padding:"3px 10px", borderRadius:20, background:"rgba(16,185,129,0.1)", color:"#6ee7b7", border:"1px solid rgba(16,185,129,0.2)", fontWeight:600 }}>{req.assetType}</span></td>
                      <td style={{ padding:"13px 16px", fontSize:12, color:T.muted, maxWidth:240, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{req.reason}</td>
                      <td style={{ padding:"13px 16px" }}><StatusBadge status={req.status} /></td>
                      <td style={{ padding:"13px 16px", fontSize:11, color:T.muted, fontFamily:"'JetBrains Mono',monospace", whiteSpace:"nowrap" }}>{req.createdAt?req.createdAt.replace("T"," ").substring(0,16):"—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{ padding:"13px 20px", borderTop:`1px solid ${T.border}`, display:"flex", justifyContent:"space-between" }}>
              <span style={{ fontSize:12, color:T.muted }}>Showing {filtered.length} of {requests.length}</span>
